...
/**
 * Advanced Student Manager for Teacher Portal (Permission Based)
 * Clone of AdminStudents with teacher-specific context
 */
export default function TeacherStudents() {
  ...
  const [availableSectionsForForm, setAvailableSectionsForForm] = useState([]) 
  useEffect(() => {
    if (!form.class_id) {
  ...
  const handleBlock = (id) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          await studentsApi.update(id, { status: 'Blocked' })
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Teacher View — Manage student records and lifecycle</p>
        </div>
