...
import { eventApi, teacherApi, classesApi } from '../../services/schoolApi'
...
export default function AdminCompetitions() {
  ...
  useEffect(() => {
    if (form.event_type === 'class_competition' && form.classPrefix && form.section && form.subject) {
      ...
      const matches = dbTeachers.filter(t => {
        ...
        const classMatch = t.assignedClasses?.some(c => c.includes(targetClass))
        const subjectMatch = t.subject === form.subject
  ...
  }, [form.event_type, form.classPrefix, form.section, form.subject, dbTeachers])

  const fetchInitialData = async () => {
    // Fetch classes independently
    try {
    ...
    try {
      ...
      const teachersData = teachersRes.data?.items || teachersRes.data?.teachers || teachersRes.data?.data?.items || teachersRes.data || []
      setDbTeachers(Array.isArray(teachersData) ? teachersData : [])
    } catch (err) {
      ...
      <div className="bg-slate-100 p-1.5 rounded-xl border border-slate-200/60 inline-flex">
        {[
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === tab.id ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
          >
            {tab.label}
          </button>
            ...
            <div className="space-y-3">
              ...
              {form.event_type === 'class_competition' && (
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="label text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 block ml-1">Assigned Teacher</label>
                    {matchedTeachers.length > 1 ? (
                      <select className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:border-brand-500" value={form.assigned_teacher} onChange={e => setForm({ ...form, assigned_teacher: e.target.value })}>
                        <option value="">Select a Teacher</option>
                        {matchedTeachers.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
                      </select>
                    ) : (
                      <input
                        className={clsx("w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none cursor-not-allowed font-semibold", form.assigned_teacher ? "bg-brand-50 text-brand-700 border-brand-200" : "bg-slate-50 text-slate-400")}
                        readOnly
              ...
              <FormInput type="textarea" rows={2} label="Description (Optional)" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-slate-100 p-1.5 rounded-xl border border-slate-200/60 inline-flex w-full mb-2">
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${modalMode === 'participants' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${modalMode === 'winners' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
              >
                Declare Winners
              </button>
