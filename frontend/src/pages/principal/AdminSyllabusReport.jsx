...
export default function AdminSyllabusReport() {
  ...
  const reportData = useMemo(() => {
    ...
    const getClassSortValue = (cls) => {
      if (!cls) return 999;
      const lower = cls.toString().toLowerCase().trim();
      
      for (const [key, val] of Object.entries(classOrder)) {
      ...
      return 100 + lower.charCodeAt(0);
    };

    const sortRows = (rows) => {
      return [...rows].sort((a, b) => {
        const classValA = getClassSortValue(a.class);
        const classValB = getClassSortValue(b.class);
        if (classValA !== classValB) {
          return classValA - classValB;
        }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4 print:hidden">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-700 disabled:bg-slate-300 text-white font-bold px-4 py-2 rounded-xl shadow-sm text-xs transition-colors h-[38px] sm:h-[42px] cursor-pointer"
          >
            {downloading ? (
