...
import { Mail, CheckCircle, Trash2, Search, Filter, Eye, Clock, Loader2 } from 'lucide-react';
...
export default function AdminContact() {
  ...
  const filteredMessages = messages.map(m => ({
    ...m,
    status: m.status || 'new' // Fallback for missing status
  })).filter(m => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const arr = Array.isArray(res) ? res : Array.isArray(res?.data) ? res.data : (res?.data?.items || res?.items || []);
      setMessages(arr);
    } catch (err) {
      ...
      <div className="card p-6">
         ...
         {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
               ...
               <p className="text-slate-500 font-medium">Loading real messages...</p>
            </div>
         ) : (
