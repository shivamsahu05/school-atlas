...
const FiltersBar = ({ 
  ...
  showWeek = false, 
  showMonth = false,
  weekOptions = [] // New prop for dynamic weeks
}) => {
  ...
  return (
    <div className="flex flex-wrap gap-2 sm:gap-3 items-center bg-white p-3 sm:p-4 rounded-xl shadow-sm border">
      <select
        value={currentClass}
        onChange={(e) => {
      ...
      {showWeek && (
        <select
          ...
          className="border rounded-lg px-3 py-2 text-sm"
        >
          <option value="Current">Select Week</option>
          {weekOptions.length > 0 ? (
            weekOptions.map(w => <option key={w.value || w} value={w.value || w}>{w.label || w}</option>)
          ) : (
            <>
