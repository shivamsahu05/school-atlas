...
const getMe = async (req, res) => {
  const user = await prisma.users.findUnique({
    ...
    select: {
      ...
      status: true, phone: true, created_at: true,
      teacher_profile: true,
    },
  })
  if (!user) return sendError(res, 'User not found.', 404)

  // Flatten teacher profile into response if it exists
  ...
  if (user.role === 'teacher') {
    const [permRows] = await pool.execute(`
      ...
      JOIN modules m ON tmp.module_id = m.id
      JOIN teachers t ON tmp.teacher_id = t.id
      WHERE t.user_id = ? AND tmp.status = 'ACTIVE' AND (tmp.end_date >= CURDATE() OR tmp.end_date IS NULL)
    `, [user.id])
    
    console.log(`[AUTH ME DEBUG] Found ${permRows.length} permissions for user ${user.id}`);
