...
exports.getResolvedTopic = async (req, res) => {
  ...
  try {
    ...
    // 2. Search with fallback for subject names (Maths/Mathematics)
    const [rows] = await pool.query(`
      SELECT s.topic FROM syllabus s
      ...
      AND (
        ...
        OR
        (REPLACE(LOWER(s.week), ' ', '') LIKE REPLACE(LOWER(CONCAT('%', ?, '%')), ' ', ''))
        OR
        (s.month IS NULL AND REPLACE(LOWER(s.week), ' ', '') = REPLACE(LOWER(?), ' ', ''))
      )
...
exports.getTeacherLOAnalytics = async (req, res) => {
  ...
  try {
    ...
    // 2.5 Fetch Micro-Schedule Tracking Defaulters with matching filters
    let trackingFilterSql = "WHERE mst.teacher_id = ? AND mst.status = 'NOT_COMPLETED'";
    let trackingParams = [teacherId];

    if (class_id && class_id !== 'undefined' && class_id !== 'null' && class_id !== '') { 
      trackingFilterSql += " AND mst.class_id = ?"; 
      trackingParams.push(Number(class_id)); 
    }
    if (section_id && section_id !== 'undefined' && section_id !== 'null' && section_id !== '') { 
      trackingFilterSql += " AND mst.section_id = ?"; 
      trackingParams.push(Number(section_id)); 
    }
    if (subject_id && subject_id !== 'undefined' && subject_id !== 'null' && subject_id !== '') { 
      trackingFilterSql += " AND mst.subject_id = ?"; 
      trackingParams.push(Number(subject_id)); 
    }

    const [trackingRows] = await pool.query(`
      SELECT mst.*, std.name, std.roll_no
      FROM micro_schedule_tracking mst
      JOIN students std ON mst.student_id = std.id
      ${trackingFilterSql}
    `, trackingParams);
    ...
    const [assigned] = await pool.query(`
      SELECT DISTINCT 
        s.class_id, ac.class_number as class_name, 
        s.section_id, asec.name as section_name,
        s.subject_id, sub.name as subject_name
      FROM syllabus s
      LEFT JOIN academic_classes ac ON s.class_id = ac.id
      LEFT JOIN acad_sections asec ON s.section_id = asec.id
      LEFT JOIN subjects sub ON s.subject_id = sub.id
      WHERE s.teacher_id = ? AND s.class_id IS NOT NULL AND s.subject_id IS NOT NULL
    `, [teacherId]);
